...
const ReturnOfDate: React.FC<ReturnOfDateProps> = ({ watch, setValue }) => {
  const returnOfDate = watch('returnOfDate')
            ...
            >
              {returnOfDate ? (
                <span className='text-lg text-blue-900'>
                  {formatDate(new Date(returnOfDate))}
                </span>
              ) : (
                <span className='text-lg text-gray-500'>(Return date)</span>
              )}
            </Popover.Button>
            <Transition
              as={Fragment}
                ...
                <div className='overflow-hidden rounded-lg border border-gray-200'>
                  <div className='flex flex-row items-center justify-between relative gap-8 bg-white px-4 py-2'>
                    <DayPicker
                      ...
                      toYear={2050}
                      selected={
                        returnOfDate ? new Date(returnOfDate) : undefined
                      }
                      onSelect={(date) => {
                        setValue('returnOfDate', date), close()
                      }}
                      numberOfMonths={1}
                      pagedNavigation
                    />
